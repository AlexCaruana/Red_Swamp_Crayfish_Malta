...
### Data Analysis code written by Alex Caruana
        ...
        CF_Distribution <- ggplot() +
                geom_histogram(data = CF[CF$ Sex == "Female",], aes(x = Carapace_Length, y = -..count..,), 
                               bins=50, color = '#2b2d42', fill = '#2b2d42', alpha = 0.5) +
                geom_histogram(data = CF[CF$ Sex == "Male",], aes(x = Carapace_Length, y = ..count..,), 
                               bins=50, color = '#8d99ae', fill = '#8d99ae', alpha = 0.5) +
                ...
                theme(axis.title.x = element_text(margin = margin(t = 15, r = 0, b = 0, l = 0)))
                        ...
                        # Supplementary Information 2 - Figure 1
                        CF_Distribution
        ...
        CF_Gender <- ggplot(data = gender_summary, mapping = aes(x = "", y = Percent, fill = Sex)) + 
                ...
                theme_void()
                        ...
                        # Supplementary Information 2 - Figure 2
                        CF_Gender
        
        ## Environmental Data Analysis
        EnvData <- read_excel("Site Environmental Data.xlsx", sheet = 2)
        EnvDataFilter <- EnvData[EnvData$Crayfish_CPUE > 0.001,] #Filtering dataset so that rows with CPUE are analysed
        EnvDataFilter$Stream_Depth <- as.numeric(EnvDataFilter$Stream_Depth)
        
                # Stream Depth & Crayfish CPUE scatter plot with regression
                        # R2 value function generated from https://groups.google.com/forum/#!topic/ggplot2/1TgH-kG5XMA
                        lm_eqn_depth <- function(EnvDataFilter){
                                m <- lm(Stream_Depth ~ Crayfish_CPUE, EnvDataFilter);
                ...
                Depth <- ggplot(EnvDataFilter, aes(x=Stream_Depth, y=Crayfish_CPUE, color = Site_Classification)) + 
                        geom_point() +
                        ...
                        geom_text(x = 67, y = 20, label = lm_eqn_depth(EnvDataFilter), parse = TRUE)

                # Stream Width & Crayfish CPUE scatter plot with regression
                        # R2 value function generated from https://groups.google.com/forum/#!topic/ggplot2/1TgH-kG5XMA
                        lm_eqn_width <- function(EnvDataFilter){
                                m <- lm(Stream_Width ~ Crayfish_CPUE, EnvDataFilter);
                                eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
                                                 list(a = format(unname(coef(m)[1]), digits = 2),
                                                      b = format(unname(coef(m)[2]), digits = 2),
                                                      r2 = format(summary(m)$r.squared, digits = 3)))
                ...
                Width <- ggplot(EnvDataFilter, aes(x=Stream_Width, y=Crayfish_CPUE, color = Site_Classification)) + 
                        geom_point()+
                        ...
                        #geom_text(x = 500, y = 16.5, label = eq(EnvDataFilter$Stream_Width,EnvDataFilter$Crayfish_CPUE), parse = TRUE) +
                        theme(legend.position = "none") +
                        geom_text(x = 550, y = 17.5, label = lm_eqn_width(EnvDataFilter), parse = TRUE)

                # Riparian Vegetation & Crayfish CPUE scatter plot with regression
                EnvDataFilter$Riparian_Vegetation_Height <- as.numeric(EnvDataFilter$Riparian_Vegetation_Height) 
                        # R2 value function generated from https://groups.google.com/forum/#!topic/ggplot2/1TgH-kG5XMA
                        lm_eqn_riparian <- function(EnvDataFilter){
                                m <- lm(Riparian_Vegetation_Height ~ Crayfish_CPUE, EnvDataFilter);
                                eq <- substitute(italic(y) == a + b %.% italic(x)*","~~italic(r)^2~"="~r2, 
                                                 list(a = format(unname(coef(m)[1]), digits = 2),
                                                      b = format(unname(coef(m)[2]), digits = 2),
                                                      r2 = format(summary(m)$r.squared, digits = 3)))
                ...
                Riparian <- ggplot(EnvDataFilter, aes(x=Riparian_Vegetation_Height, y=Crayfish_CPUE, color = Site_Classification)) + 
                        geom_point()+
                        #stat_poly_line(color = "#448471") +
                        #stat_poly_eq(use_label(c("eq", "R2"))) +
                        geom_smooth(method=lm, na.rm = TRUE, fullrange= TRUE,
                        ...
                        xlim(0, 150) +
                        scale_y_continuous(breaks=seq(0,20,2.5)) +
                        #geom_text(x = 50, y = 15.7, label = eq(EnvDataFilter$Riparian_Vegetation_Height,EnvDataFilter$Crayfish_CPUE), parse = TRUE) +
                        theme(legend.position = "none") +
                        geom_text(x = 37, y = 15, label = lm_eqn_riparian(EnvDataFilter), parse = TRUE)
                ...
                EnvDataFilter$Aquatic_Vegetation_Cover <- as.numeric(EnvDataFilter$Aquatic_Vegetation_Cover)  
                EnvDataFilter$Aquatic_Vegetation_Cover_Breaks <- cut(EnvDataFilter$Aquatic_Vegetation_Cover, breaks=5, include.lowest=TRUE)
        ...
        ## T-Test
                #Testing Gender      
                testF <- WS[WS$Sex=="Female",]
                testM <- WS[WS$Sex=="Male",]
                t.test(testF$Carapace_Length, testM$Carapace_Length, var.equal = FALSE)
                var.test(testF$Carapace_Length, testM$Carapace_Length, alternative = "two.sided")
                
                #Testing Excavated vs Non-excavated sites            
